program euler2D
  ...
  integer:: ndim,nzones,nbtypes,nnodes,nedges,ncells,maxppf,maxfpc,Frame
...
Frame = 0
  ...
  open(33,file='frames/bow.dat') !,form='unformatted')
  !open(34,file="density",form='formatted')

  write(33,*) "title = sample"
  write(33,*)  "variables = x,y,d,u,v,p"
  write(33,*)  'zone nodes=',nnodes,',elements=',ncells,',DATAPACKING=BLOCK,ZONETYPE=FETRIANGLE,','varlocation=([3,4,5,6]=cellcentered),solutiontime=',0
  write(33,*)  (nodexy(1,i),i=1,nnodes)
  write(33,*)  (nodexy(2,i),i=1,nnodes)


  !Start the time loop 
  !=======================
  iter=0
  do while(rms>1.0e-5 .and. iter<35000)!while(iter<maxiter .and. maxR>Rtol)
     iter=iter+1
...
112 Format(I4.4)
  ...
  if(Frame .GE. 2) then
  Write(33,*) 'zone nodes=',nnodes,',elements=',ncells,',DATAPACKING=BLOCK,ZONETYPE=FETRIANGLE,','varlocation=([3,4,5,6]=cellcentered),Solutiontime =',Frame-1,'varsharelist = ([1-2])'
  end if
  !WRITE(33) (nodexy(1,i),i=1,nnodes)
  !write(33) (nodexy(2,i),i=1,nnodes)
  write(33,*) (density(i),i=1,ncells)
  write(33,*) (u_vel(i),i=1,ncells)
  write(33,*) (v_vel(i),i=1,ncells)
  write(33,*) (pressure(i),i=1,ncells)
  do i=1,ncells
